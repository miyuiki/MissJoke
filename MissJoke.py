...
class PttWebCrawler(object):
    ...
    def __init__(self, board, page, pu):
        ...
        for i in range(end-start+1):
            ...
            for div in divs:
                try:
                    ...
                    pushhh = str(div.select('span'))
                    pushhh = pushhh.split('>')
                    pushhh = pushhh[1].split('<')
                    if int(pushhh[0]) >= int(pu):
...
if __name__ == '__main__':
    board = input("Plz enter the board you want to listen to? ")
    page = input("How many pages do you want to listen to? ")
    pu = input("how many push do you want? ")
    c = PttWebCrawler(board, page, int(pu))
    mixer.init()
    for i in joke:
        print(i)
        with tempfile.NamedTemporaryFile(delete = True) as fp:
            tts = gtts.gTTS(text = i, lang = 'zh')
            tts.save('{}.mp3'.format(fp.name))
            mixer.music.load('{}.mp3'.format(fp.name))
            mixer.music.play()
            time.sleep(len(i)*3/10)
